import { a as AGY_VERSION_FALLBACK, g as isProxyRouted, h as AgyPoolBlockedError, m as AgyAuthError, p as resolveAgyClientCredentials, s as OAUTH_TOKEN_URL, t as AGY_CLIENT_ID } from "./constants-BFKYjOmh.mjs";
import { a as proxiedFetch, r as isProxyUnreachableError, t as accountFetch } from "./proxy-DLvX8yxv.mjs";
import { t as deriveAntigravitySessionId } from "./identity-DsftHTJC.mjs";
		...
		const updates = (await Promise.all(stale.map(async (account) => {
			...
			const refresh = (async () => {
				try {
					...
					if (!auth) return null;
					const { fetchAvailableModels } = await import("./models-BP9kYRcK.mjs").then((n) => n.r);
		...
		*/
		...
		for (let attempt = 0; attempt < maxAttempts; attempt++) {
			...
			if (!picked.account.projectId && !this.projectRetryFailed.has(key)) try {
				const { loadCodeAssist } = await import("./exchange-dQm2mjJ5.mjs").then((n) => n.n);
	...
	*/
	async testCall(model, prompt = "Reply with exactly: OK", maxTokens = 1024) {
		try {
			...
			const { toAgyRequestBody } = await import("./translate-D_o2YANO.mjs").then((n) => n.n);
			const { fetchAgyFirstOk } = await import("./constants-BFKYjOmh.mjs").then((n) => n.l);
...
//#endregion
export { classifyHttpError as _, deriveKey as a, resolveDshHome as c, isAgyDisabled as d, generateFingerprint as f, classifyFetchError as g, resolveAntigravityVersionBounded as h, createAesGcmCodec as i, resolveMasterKeyCodec as l, resolveAntigravityVersion as m, maskProxyUrl as n, loadMasterKey as o, recordFingerprintVersion as p, MASTER_KEY_REF as r, persistMasterKey as s, JsonAccountStore as t, AgySessionManager as u, describeFetchError as v };

//# sourceMappingURL=accounts-wzH9KGR7.mjs.map