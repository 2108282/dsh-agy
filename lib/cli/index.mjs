import { n as AGY_DEFAULT_REDIRECT_URI } from "../constants-Db4tmyfb.mjs";
import { i as normalizeProxyUrl, n as isProxyReachable, o as proxiedFetch, t as accountFetch } from "../proxy-DfaL73mL.mjs";
import { _ as resolveMasterKeyCodec, c as resolveAntigravityVersionBounded, f as createAesGcmCodec, g as resolveDshHome, i as isAgyDisabled, m as loadMasterKey, n as maskProxyUrl, p as deriveKey, r as AgySessionManager, t as JsonAccountStore } from "../accounts-D0KoucnO.mjs";
...
async function loginCommand(options) {
	...
	const versionProbe = resolveAntigravityVersionBounded(1500, normalizedProxy ? accountFetch({ proxyUrl: normalizedProxy }) : proxiedFetch);
	let code;
	let state;
	if (options.headless) {
	...
	await versionProbe;
	const result = await exchangeAntigravity(code, state, redirectUri, verifier, { ...normalizedProxy ? { proxyUrl: normalizedProxy } : {} });
...
async function statusCommand() {
	...
	for (const [index, account] of storage.accounts.entries()) {
		...
		if (session && session.index === index) try {
			...
			const discovered = await fetchAvailableModels(session.auth.access, session.account.projectId, accountFetch({ proxyUrl: session.account.proxy }));
...
*/
function writeBlobFile(file, blob) {
