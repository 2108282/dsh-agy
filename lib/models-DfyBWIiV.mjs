...
import { D as getAgyBootstrapUserAgent, b as AGY_ENDPOINT_FALLBACKS } from "./accounts-Cle7tmMg.mjs";
import { i as proxiedFetch } from "./proxy-DQPIwUov.mjs";
import { a as isLevelThinkingModel, i as isClaudeModel, n as catalogModel, o as resolveModelAlias, r as isChatCallableModelId, t as AGY_PUBLIC_MODELS } from "./catalog-DX6YaSY3.mjs";
...
function mergeModelCatalog(dynamic) {
	...
	for (const [rawId, entry] of Object.entries(dynamic.models ?? {})) {
		...
		const displayName = (entry.displayName && entry.displayName !== rawId ? entry.displayName : void 0) ?? meta?.name ?? entry.displayName ?? entry.modelName ?? id;
		entries.push({
			provider: "agy",
			id,
			name: displayName,
...
function resolveAgyModel(provider, model) {
	...
	const defaultMaxTokens = meta?.maxOutputTokens ?? (isClaudeModel(model) ? 64e3 : void 0);
	return {
		provider,
		id: model,
		name: meta?.name ?? model,
...
//# sourceMappingURL=models-DfyBWIiV.mjs.map