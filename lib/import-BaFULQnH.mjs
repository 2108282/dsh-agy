import { a as AGY_PLATFORM_ENUM, c as OAUTH_AUTHORIZE_URL, g as resolveAgyClientCredentials, h as getAgyBootstrapUserAgent, i as AGY_IDE_TYPE, o as AGY_SCOPES, p as currentAgyVersion, r as AGY_ENDPOINT_FALLBACKS } from "./constants-Db4tmyfb.mjs";
import { i as normalizeProxyUrl, o as proxiedFetch } from "./proxy-DfaL73mL.mjs";
import { i as generatePkcePair, r as encodeState } from "./exchange-yZPIZumk.mjs";
...
async function enrichWithAntigravityBackend(parsed, options = {}) {
	let email = null;
	let projectId = null;
	...
	try {
		...
		const loadBody = JSON.stringify({ metadata: {
...
async function parseImportSource(source, kind, options = {}) {
	if (kind === "blob") {
...
async function importManySources(items, store, options = {}) {
	...
	for (const item of items) try {
		const { created } = await upsertImportedAccount(store, await parseImportSource(item.source, item.kind, { ...options.proxy ? { proxyUrl: normalizeProxyUrl(options.proxy) } : {} }), options);
...
//# sourceMappingURL=import-BaFULQnH.mjs.map