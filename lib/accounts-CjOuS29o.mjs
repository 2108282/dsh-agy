...
function persistMasterKey(dshHome, masterKey) {
	...
	let existingText = existsSync(file) ? readFileSync(file, "utf8") : "";
	if (existingText.length > 0 && readCredentialsDocument(file).has("AGY_MASTER_KEY")) return;
	let next;
	if (existingText.includes("refs:")) {
		next = existingText.replace("refs:", `refs:\n  ${MASTER_KEY_REF}: ${JSON.stringify(masterKey)}`);
	} else if (existingText.trim().length > 0) {
		next = existingText + `\nrefs:\n  ${MASTER_KEY_REF}: ${JSON.stringify(masterKey)}\n`;
	} else {
		next = `version: 1\nrefs:\n  ${MASTER_KEY_REF}: ${JSON.stringify(masterKey)}\nrecords: {}\n`;
	}
