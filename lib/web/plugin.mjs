import { d as isAgyDisabled, f as generateFingerprint, g as resolveAntigravityVersionBounded, n as maskProxyUrl, p as recordFingerprintVersion } from "../accounts-Cle7tmMg.mjs";
import { a as proxyUrlForLogs, r as normalizeProxyUrl, t as isProxyReachable } from "../proxy-DQPIwUov.mjs";
import { t as createAgyRuntime } from "../plugin-common-CsWGn6kD.mjs";
import { t as exchangeAntigravity } from "../exchange-COICrev-.mjs";
import { n as upsertImportedAccount, r as authorizeAntigravity, t as importManySources } from "../import-CB7Zj64D.mjs";
...
function createAgyWebRoutes(options) {
	...
	const accountsList = async () => {
		...
		for (const entry of list) {
			...
			try {
				const { fetchAvailableModels } = await import("../models-DfyBWIiV.mjs").then((n) => n.r);
	...
	const handleModels = async (_req, res) => {
		...
		try {
			const { listAgyModels } = await import("../models-DfyBWIiV.mjs").then((n) => n.r);
