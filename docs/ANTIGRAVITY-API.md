...
- **Explicit numeric budgets ARE accepted by this channel** (relevant to any future budget setting): `{thinkingBudget:0}` → 200, thinking off (`thoughtsTokenCount` 0); `8192` → 200, ~530; `32768` → 200, ~655–797; and `thinkingLevel` low/high moves `thoughtsTokenCount` (~86–124 vs ~205–229 on tiered). `{thinkingLevel:"auto"}` → **400** `Invalid value at 'request.generation_config.thinking_config.thinking_level'`, so the level vocabulary is closed to low/medium/high.
