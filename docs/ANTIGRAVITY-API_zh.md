...
- **该通道接受显式数值预算**（与未来的预算设置相关）：`{thinkingBudget:0}` → 200，思考关闭（`thoughtsTokenCount` 0）；`8192` → 200，约 530；`32768` → 200，约 655–797；`thinkingLevel` low/high 也会移动 `thoughtsTokenCount`（tiered 上约 86–124 对 205–229）。`{thinkingLevel:"auto"}` → **400** `Invalid value at 'request.generation_config.thinking_config.thinking_level'`，故档位词表封闭为 low/medium/high。
