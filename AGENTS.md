...
- **Version Freshness**:
  ...
  - The platform token is PINNED to `darwin/arm64` (the reference build the backend expects; two independent implementations of this client converged on pinning it). The removed `windows/amd64` / `darwin/amd64` entries were Go-style tokens an Electron client does not emit — if multi-platform is ever restored, the correct tokens are `win32/x64` and `darwin/x64`, and that needs a capture first.
  - The freshness gate also cross-checks the separate `antigravity-hub` updater line and WARNS when it disagrees (today 2.15.1 vs the IDE line's 2.0.0), because which line the backend validates is unproven — do not "fix" the fallback to the hub value without the A/B measurement described in the gate's output.
...
- `scripts/` contains developer tools (`record:fixtures`, `e2e`, `debug:request`, `verify:tools`, `verify:blocks`, `verify:claude-cap`, `verify:claude-parts`, `verify:proxy-routing`, `verify:version-freshness`), **all requiring real accounts or network access**. They are not part of routine dev loops, are not packaged into npm, and are not run in CI. `scripts/unfold-credentials.mjs` is the exception: a standalone stopgap for the published 0.2.7 reader, run directly with node.
...
- No client-identifying header is SYNTHESIZED beyond the impersonation set: `x-client-name` / `x-client-version` / `x-machine-id` / `x-vscode-sessionid` (present in another implementation) are deliberately absent, because inventing their values is a new anomaly while a subset is merely incomplete — a capture of the official client is the only thing that should add them.
- `buildRequestHeaders` sends exactly ONE `User-Agent`, and it is the Antigravity client string. The harness's `attributionHeaders()` is deliberately NOT merged in: it returns a lowercase `user-agent` while the impersonation object uses camel-case, so spreading both produced two properties that `fetch` folded into one comma-joined header naming this tool on every request. `tests/adapter.test.ts` asserts the wire value, not the intermediate object.
