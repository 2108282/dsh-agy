...
- **Version Freshness**:
  ...
  - The release feeds belong to no account but their request still egresses HOST: the plugin probes through the pool's representative account (`pickProbeProxyUrl`) and the CLI through the login proxy, never a bare env/direct route.
...
- NEVER feed a local cooldown into `providerRetryAfterMs`: DSH's policy is `mode: 'normal'` with a 10s `maxDelayMs`, and `llm-retry` calls `next()` — giving up on the turn — when the value exceeds it. The account-level park is the cooldown; the retry is DSH's.
