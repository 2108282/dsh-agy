...
- **Client Presentation (`src/client/`)**:
  ...
  - An RPC whose failure is IN BAND (`{ ok: false }`: `account.verify` / `account.test` / `account.proxyTest`) must be read by its caller — `act()` sees only a rejection and then `refresh()` clears the banner, so routing one through it discarded the verdict AND erased it (the user saw the busy flash and nothing else); `runAction` is the envelope for those, `act` stays for throw-signalled mutations.
  - `actionError` is a SEPARATE channel from `error`: only a one-shot action's verdict expires (`ACTION_MESSAGE_TTL_MS`), because a standing failure like "the account list did not load" must persist or a broken page looks fine 3.5s later.
  - `.agy-error` needs its own `white-space: pre-wrap`, or the default folds the newline in a `what failed\nwhy` message.
  - `t()` substitutes `{name}` only when the key is present and otherwise returns the match unchanged, so a forgotten argument renders literal braces instead of failing; a test walks every placeholder against its call sites.
  - The Usage headline folds the prompt side (`promptTokens` = uncached + cacheRead + cacheWrite) and shows the cache line as a HIT count, a subset relation the reader never adds — while the LEDGER keeps `input` uncached-only (DSH's disjoint-bucket vocabulary), so the fold is display-only.
  - The composition bar must stay a PARTITION of `totalTokens`, which is why its middle row is a dedicated `未命中` key: reusing the headline's input label there double-counts the cache to 177%.
...
- **Upstream Wire Facts** (`docs/ANTIGRAVITY-API.md`, verified empirically):
  ...
  - `thinkingConfig.thinkingBudget` is accepted over `[-1, 65535]` and `-1` IS accepted (adaptive need not be expressed by omission, contrary to another implementation's changelog), while `minThinkingBudget` is NOT a validation and must never become a client-side clamp; `pnpm run verify:thinking-config` re-measures that, and separately asserts the CONTRADICTION that an id-bound model does not actually reject a `thinkingLevel` with 400 as `docs/ANTIGRAVITY-API.md` §3 claims.
...
- `scripts/` contains developer tools (`record:fixtures`, `e2e`, `debug:request`, `verify:tools`, `verify:blocks`, `verify:claude-cap`, `verify:claude-parts`, `verify:thinking-config`, `verify:proxy-routing`, `verify:version-freshness`, `verify:official-identity`, `verify:metadata-acceptance`), **all requiring real accounts or network access**. They are not part of routine dev loops, are not packaged into npm, and are not run in CI. `scripts/unfold-credentials.mjs` is the exception: a standalone stopgap for the published 0.2.7 reader, run directly with node.
...
- **Fingerprint regeneration has an INVERTED gate** (known defect, recorded in `session.ts` and deliberately unfixed): it is gated on the `rate-limit` branch, so it never fires on `verification-required` — the one signal that means upstream actually gated the account — and only two of the five `Fingerprint` fields (`userAgent`, `apiClient`) ever reach a request header, so "a new identity" re-rolls two header values rather than the device fields the name suggests.
...
- **Reasoning efforts MUST NOT declare `defaultEffort`.** The harness resolves `effective = requested ?? reasoning.defaultEffort` and gates the selector's "provider default" (adaptive) entry on `defaultEffort === void 0`, so declaring one both forces an effort onto every request and deletes the only option meaning "let the model decide" — the symptom is a selector with no adaptive entry whose every request carries `thinkingConfig`.
- **The model quota card is gone; the grouped 5h/weekly windows are the surviving quota display.** Its per-model `quotaInfo` rows were replaced by `cachedLimits` (grouped, from `retrieveUserQuotaSummary`) — that endpoint reports only `remainingFraction`/`resetTime` and cannot express a window, so the card could not be improved into what the limits card already is. Do not restore `account.quota`: its upstream probe rode the accounts view and is the reason the list once rendered behind a spinner.
- **A configured `thinkingBudget` for Claude needs `max_tokens` STRICTLY greater than it** (measured: `budget=1024, max_tokens=1024` is a 400, as is a budget with no `maxOutputTokens`), and Claude's floor is 1024 rather than `-1`; a budget without room is DROPPED, never forced through by raising the caller's cap.
- **Tiered detection is a KNOWN GAP (issue #47)**: `isLevelThinkingModel` keys off the `-tiered` id suffix, which is a naming coincidence, so upstream's own `tieredModelIds` grouping is ignored — `pro` (`gemini-3.1-pro-low`, which does accept `thinkingLevel`) and `flashLite` are excluded, and a future tiered model under another name fails silently. Issue #47 carries the measurements; do not re-probe them.
- **A thinking budget is a CAP, not a target** (measured): on an easy prompt every tier looks identical because none of them bind, and the difference only appears once the task saturates the cap (same hard prompt: 8192 -> ~2500 thoughts, 65535 -> ~24000). Any comparison of budgets must use a task that saturates, or it will wrongly conclude the setting does nothing.
- **`budgetFor` is on the generation hot path and must not read the file per request**: revalidation is throttled (`DEFAULT_REVALIDATE_INTERVAL_MS`), and the WRITE path bypasses the throttle so a concurrent edit is never merged into a stale copy.
- **The 5h/weekly window refresh must NEVER write `cachedQuota`.** That cache feeds `rankPoolCandidates`, which turns a measured `remainingFraction <= 0` into a `blockedUntil`, so it is gated to pools of >1 (`getSession`) precisely because blocking a SOLO account leaves no fallback (`AgyPoolBlockedError`); `refreshLimits` exists so the display path works at any pool size, and it is the only thing that fills `cachedLimits` for a single account.
- **`force` on `refreshLimits` changes WHICH accounts are probed, never WHERE the result lands** — it exists so the toolbar's Refresh can beat the 10-minute `LIMITS_CACHE_TTL_MS`, and it must not become a second path into `cachedQuota`.
- **`refreshLimits` returns what it did (`measured`/`failed`/`skipped`) because a forced probe that fails writes nothing at all** — no cache, no `updatedAt` — so "probed and failed" and "still fresh" are otherwise indistinguishable and the click looks inert.
- **A configured `thinkingBudget` REPLACES `thinkingLevel`, never joins it** (measured: with both present the level wins — `low`+16000 spends what `low` alone spends, `high`+1000 what `high` does), and an unset budget must stay a no-op so the shipped default changes no request.
