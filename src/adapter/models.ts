...
import { AGY_PUBLIC_MODELS, catalogModel, isChatCallableModelId, isLevelThinkingModel, resolveModelAlias } from './catalog.ts'
...
export function mergeModelCatalog(dynamic: DiscoveredModels): LlmModelInfo[] {
  ...
  for (const [id, entry] of Object.entries(dynamic.models ?? {})) {
    ...
    if ((entry as any).isInternal === true || (entry as any).apiProvider === "API_PROVIDER_INTERNAL") continue
