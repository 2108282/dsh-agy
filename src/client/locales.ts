...
export const zh = {
  ...
  // Describes what the USER SEES in the picker, not how we detect it: the id
  // suffix is an implementation detail nobody can check from the UI.
  thinkingGeminiHint: '为每个思考等级设定思考量上限。留空 = 由上游决定上限；填数值 = 自定上限，越大允许想得越多（{min}–{max}）。',
  thinkingTieredLabel: 'Default',
  thinkingSamplesTitle: '实测思考量参考',
  thinkingSamplesLevel: '等级',
  thinkingSamplesEasy: '简单题',
  thinkingSamplesMedium: '中等题',
  thinkingSamplesHard: '困难题',
  thinkingSamplesNote: '实测样本，非固定值：思考量随难度浮动，同一档位在简单题与困难题上可相差上百倍。“困难题”为四道竞赛题、要求完整推导。把预算填到上限只会略高于 High（中等题约 +16%），因为上限并不决定实际思考量。',
...
export const en: Record<AgyLocaleKey, string> = {
  ...
  thinkingChipMax: 'Max',
  thinkingGeminiGroup: 'Gemini',
  thinkingGeminiHint: 'Set a thinking cap per effort. Blank = upstream sets the cap; a value sets your own ({min}–{max}) — higher allows more thinking.',
  thinkingTieredLabel: 'Default',
  thinkingSamplesTitle: 'Measured thinking tokens',
  thinkingSamplesLevel: 'Effort',
  thinkingSamplesEasy: 'Easy',
  thinkingSamplesMedium: 'Medium',
  thinkingSamplesHard: 'Hard',
  thinkingSamplesNote: 'Samples, not fixed values: thinking scales with difficulty, and one effort can differ over a hundredfold between an easy and a hard prompt. "Hard" is four competition problems requiring full derivations. Raising the cap to the maximum exceeds High only slightly (about +16% on the medium prompt), because the cap does not decide actual thinking.',
  thinkingClaudeGroup: 'Claude',
