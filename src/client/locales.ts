...
export const zh = {
  ...
  thinkingGeminiHint: '给每个思考等级指定思考量，可把低档拉高，也可把高档压低。留空 = 用该档默认值（{min}–{max}）。',
  ...
  thinkingSamplesNote: '样本，非固定值：思考量随难度浮动，同一档位在简单题与困难题上可差上百倍。留空 ≠ 想得最多——困难题上 Default 低于 Medium。High 已用满约 96% 的上限，填更大的数没有额外效果。',
...
export const en: Record<AgyLocaleKey, string> = {
  ...
  thinkingGeminiGroup: 'Gemini',
  thinkingGeminiHint: 'Set the thinking amount for each effort — raise a low tier, or lower a high one. Blank uses that tier\'s default ({min}–{max}).',
  thinkingTieredLabel: 'Default',
  ...
  thinkingSamplesNote: 'Samples, not fixed values: thinking scales with difficulty, and one effort can differ over a hundredfold between an easy and a hard prompt. Blank does NOT mean maximum — on the hard prompt Default is below Medium. High already uses about 96% of the ceiling, so a larger number buys nothing.',
  thinkingClaudeGroup: 'Claude',
